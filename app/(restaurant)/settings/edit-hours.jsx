...
export default function EditHours() {
    ...
    const loadData = async () => {
        try {
            ...
            if (!user) {
                // User not authenticated - will be handled by auth guard
                return;
            }
            ...
            } else if (!existingRestaurant.success && !existingRestaurant.retryable) {
                // Only show error for non-retryable errors
                // Retryable errors will be handled silently
            }
        } catch (error) {
            // Silent error - data will use defaults
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            const user = getCurrentUser();
            if (!user) {
                // This should be handled by auth guard, but just in case
                setLoading(false);
                return;
            }
            ...
            if (!restaurant.success) {
                // Show error only for non-retryable errors
                if (!restaurant.retryable) {
                    // Could show error toast here
                    // showErrorToast(restaurant.error);
                }
                setLoading(false);
                return;
            }

            const result = await updateRestaurant(restaurant.data.id, { hours: schedule });
            if (!isMounted) return;
        ...
        <View style={styles.safeArea}>
            <View style={styles.header}>
                <View style={{ marginTop: spacing.sm }}>
                    ...
                    <Text style={styles.subtitle}>Update your business hours</Text>
                </View>
            </View>
